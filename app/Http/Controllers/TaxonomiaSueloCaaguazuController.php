...
class TaxonomiaSueloCaaguazuController extends Controller
{
    ...
    public function cargar(Request $request)
    {
        ...
        if (File::exists($rutaArchivo)) {
            ...
            $id = $request->input('id');
            $nuevoValor = $request->input('desc');
            $nuevoOrden = $request->input('orden');
            /*$id = 8;
            $nuevoValor = "Este es hina nde";
            $nuevoOrden= "Este es";*/
            $array['features'] = $this->replaceFeatures($id, $array['features'], $nuevoValor, $nuevoOrden);
            unlink($rutaArchivo);
    ...
    private function replaceFeatures($id, $features, $newVal, $newOrd)
    {
        foreach ($features as $key => $feature) {
            if ($id == $feature['properties']['ID']) {
                $features[$key]['properties']['DESC'] = $newVal;
                $features[$key]['properties']['ORDEN'] = $newOrd;
                return $features;
            }
        }
        return $features;
