...
class CropRotationService
{
    ...
    public function recommendCrops(Soil $soil): array
    {
        ...
        Log::info('Valores ideales: ' . json_encode($this->idealValues));
        ...
        $orderedCrops = collect($crops)->sortByDesc('suitability');
        Log::debug('Cultivos antes de ordenar: ' . print_r($crops, true));

        $orderedCrops = collect($crops)->sortByDesc('suitability')->toArray();
        Log::debug('Cultivos después de ordenar: ' . print_r($orderedCrops, true));

        $orderedCrops = array_keys($orderedCrops);
    ...
    private function calculateSuitability(array $actualValues): float
    {
        ...
        $phSuitability = $this->calculateSuitabilityFactor($actualValues['ph'], $this->idealValues['ph']);
        $suitability += $phSuitability;
        $moSuitability = $this->calculateSuitabilityFactor($actualValues['mo'], $this->idealValues['mo']);
        $suitability += $moSuitability;
        $al3Suitability = $this->calculateSuitabilityFactor($actualValues['al3'], $this->idealValues['al3']);
        $suitability += $al3Suitability;
        $fosforoSuitability = $this->calculateSuitabilityFactor($actualValues['fosforo'], $this->idealValues['fosforo']);
        $suitability += $fosforoSuitability;
        $precipitacion_anualSuitability = $this->calculateSuitabilityFactor($actualValues['precipitacion_anual'], $this->idealValues['precipitacion_anual']);
        $suitability += $precipitacion_anualSuitability;
        $temperaturaSuitability = $this->calculateSuitabilityFactor($actualValues['temperatura'], $this->idealValues['temperatura']);
        $suitability += $temperaturaSuitability;
        $humedadSuitability = $this->calculateSuitabilityFactor($actualValues['humedad'], $this->idealValues['humedad']);
        $suitability += $humedadSuitability;
        Log::debug("Conveniencia para ph: $phSuitability");
        Log::debug("Conveniencia para mo: $moSuitability");
        Log::debug("Conveniencia para al3: $al3Suitability");
        Log::debug("Conveniencia para fosforo: $fosforoSuitability");
        Log::debug("Conveniencia para precipitacion_anual: $precipitacion_anualSuitability");
        Log::debug("Conveniencia para temperatura: $temperaturaSuitability");
        Log::debug("Conveniencia para humedad: $humedadSuitability");
